...
mild.est = function(est.res, exp.tilt.list, exp.tilt.model.x.names, auxilliary, family = "gaussian", ortho = FALSE, true.pi = NULL, perturb = rep(1, N)){
  true.pi = rep(1, N)
  true.pi[r == 1] = est.res$pi.m%*%est.res$w.pi
  J = length(exp.tilt.list)
  full.pi.m = matrix(NA, N, J)
  for(j in 1:J){
    full.pi.m[, j] = exp.tilt.list[[j]](y, dat[exp.tilt.model.x.names], N)*true.pi
  }
  ...
  # Compress the propensity score models
  ################################################################################
  w.pi.fit = find.w.pi(auxilliary, pi.m, r, ortho = FALSE, init = rep(1/J, J), perturb = perturb)
  w.pi = w.pi.fit$theta.hat
  w.pi = as.matrix((w.pi^2)/as.numeric(t(w.pi)%*%w.pi))
  compressed.pi = rep(1, N)
  compressed.pi[r == 1] = pi.m%*%w.pi
