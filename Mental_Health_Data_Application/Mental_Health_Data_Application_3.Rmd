...
bootstrap = function(ps_specifications, data, B){
  ...
  boot_result = foreach(i= 1:B, .combine = 'cbind', .options.snow = opts, .packages = parallel_packages) %dopar% {
    tryCatch({
      ...
      c(mu_ipw = estimates[1], se_ipw = estimates[2], mu_cc = weighted.mean(data$teacher_report[data$r == 1], rexp(sum(data$r == 1))))
...
for(model_num in 1:3){
  ...
  for(i in 1:ncol(model_combinations)){
    ...
    for(k in 1:length(health)){
      ...
      save_file = paste0(c("MHD_results/EBMR_IPW_", subset_names[k], "_", model_set, "_OR3.RDS"), collapse = "")
...
for(model_num in 1:3){
  ...
  for(i in 1:ncol(model_combinations)){
    print(c(model_num, i))
    model_set0 = model_combinations[, i]
    boot_result0 = readRDS(paste0(c("MHD_results/EBMR_IPW_bootstrap_", subset_names[1], "_", model_set0, "_OR_perturb.RDS"), collapse = ""))
    ...
    for(j in 1:ncol(model_combinations)){
      model_set1 = model_combinations[, j]
      boot_result1 = readRDS(paste0(c("MHD_results/EBMR_IPW_bootstrap_", subset_names[2], "_", model_set1, "_OR_perturb.RDS"), collapse = ""))
