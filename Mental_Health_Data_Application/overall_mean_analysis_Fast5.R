...
# Using EIPS (new nu moment function)
#------------------------------------------------------------------------------#
...
#------------------------------------------------------------------------------#
ebmr <- EIPS$new("teacher_report", ps_specifications, dat, W)
...
if (file.exists(boot_file)) {
  ...
  boot_results_raw <- foreach(
    b = 1:B,
    .options.snow = opts,
    .packages = c("EIPS", "stringr", "Matrix", "numDeriv"),
    ...
    mu_list <- list()
    tryCatch({
      ebmr_b <- EIPS$new("teacher_report", ps_spec_b, dat_b, W)
      for (j in seq_along(all_model_sets)) {
        model_set <- all_model_sets[[j]]
        label <- model_set_labels[j]
...
fmt_val <- function(x) {
  if (is.na(x)) return("")
  if (x >= 0) paste0("~", sprintf("%.3f", x)) else sprintf("%.3f", x)
}
...
tex <- c(tex, paste0(I, "\\caption{Population mean estimation results for $E[\\text{teacher\\_report}]$ ",
                     "using EIPS. Analytical SE is based on the influence function. ",
...
tex <- c(tex, paste0(II, " & Estimate & Analytical SE & Bootstrap SE & 95\\% CI \\\\"))
tex <- c(tex, paste0(II, "\\midrule"))

# CC
ci_cc <- c(mu_cc - 1.96 * boot_se_trim_cc, mu_cc + 1.96 * boot_se_trim_cc)
tex <- c(tex, paste0(II, "CC & ", fmt_val(mu_cc), " & ", fmt_val(se_cc),
                     " & ", fmt_val(boot_se_trim_cc),
                     " & $[", sprintf("%.4f", ci_cc[1]), ",\\, ", sprintf("%.4f", ci_cc[2]), "]$ \\\\"))

# MAR
tex <- c(tex, paste0(II, "MAR & ", fmt_val(mu_mar), " & -- & -- & -- \\\\"))
tex <- c(tex, paste0(II, "\\midrule"))
...
for (j in seq_along(all_model_sets)) {
  label <- model_set_labels[j]
  res <- results_list[[label]]
  se_use <- boot_se_trim[label]
  ci <- c(res$mu - 1.96 * se_use, res$mu + 1.96 * se_use)
  tex <- c(tex, paste0(II, est_tex_names[j], " & ", fmt_val(res$mu),
                       " & ", fmt_val(res$se),
                       " & ", fmt_val(se_use),
