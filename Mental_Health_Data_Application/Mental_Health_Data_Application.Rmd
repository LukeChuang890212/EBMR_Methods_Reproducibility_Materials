...
for(model_num in 1:3){
  ...
  for(i in 1:ncol(model_combinations)){
    ...
    for(k in 1:length(health)){
      ...
      save_file = paste0(c("MHD_results/EBMR_IPW_", subset_names[k], "_", model_set, "_OR.RDS"), collapse = "")
...
for(model_num in 1:3){
  ...
  for(i in 1:ncol(model_combinations)){
    ...
    for(k in 1:2) {
      ...
      save.file = paste0(c("MHD_results/EBMR_IPW_bootstrap_", subset_names[k], "_", model_set, "_OR.RDS"), collapse = "")
...
for(model_num in 1:3){
  ...
  for(i in 1:ncol(model_combinations)){
    ...
    result0 = readRDS(paste0(c("MHD_results/EBMR_IPW_", subset_names[1], "_", model_set0, "_OR.RDS"), collapse = ""))
    all_estimates[1, ] = unlist(result0[c(1, 3)])
    for(j in 1:ncol(model_combinations)){
      model_set1 = model_combinations[, j]
      result1 = readRDS(paste0(c("MHD_results/EBMR_IPW_", subset_names[2], "_", model_set1, "_OR.RDS"), collapse = ""))
...
for(model_num in 1:3){
  ...
  for(i in 1:ncol(model_combinations)){
    print(c(model_num, i))
    model_set0 = model_combinations[, i]
    boot_result0 = readRDS(paste0(c("MHD_results/EBMR_IPW_bootstrap_", subset_names[1], "_", model_set0, "_OR.RDS"), collapse = ""))
    ...
    for(j in 1:ncol(model_combinations)){
      model_set1 = model_combinations[, j]
      boot_result1 = readRDS(paste0(c("MHD_results/EBMR_IPW_bootstrap_", subset_names[2], "_", model_set1, "_OR.RDS"), collapse = ""))
...
boot_result[1,] = boot_result0[3, ]
boot_result[2,] = boot_result1[3, ]
boot_mu_ipw = apply(boot_result[1:2, ], 1, function(v) mean(rm.extreme(v)))
boot_var_ipw = apply(boot_result[1:2, ], 1, function(v) var(rm.extreme(v)))
...
for(xi in xi.v){
  ...
  for(k in 1:length(health)){
    sa_result = readRDS(paste0(c("MHDResults/MNAR_est_", subset_names[k], "_", 1:3, "_mild", round(xi, 3), "_OR5.RDS"), collapse = ""))
    print(sa_result$w.pi)
    est[k, ] = unlist(mild.est.all_results[c(1, 2)])
  }
...
# Create the plot
plot(cbind(xi.v, all_results[, 1]), type = "o",
     xlab = expression(xi), 
...
points(zeta.v, all_results[, 1], pch = 16)
abline(h = 1, lwd = 2)
```
...
```{r}
# boot_result = readRDS("MHDResults/MNAR_est_boot_health_effect_OR2.RDS")
# par(mfrow = c(2, 4))
# for(i in 1:nrow(boot_result)) hist(boot_result[i, ])
```
...
for(model_num in 1:3){
  ...
  for(i in 1:ncol(model_combinations)){
    print(c(model_num, i))
    model_set0 = model_combinations[, i]
    boot_result0 = readRDS(paste0(c("MHDResults/MNAR_boot_est_", subset_names[1], "_", model_set0, "_mild_OR13.RDS"), collapse = ""))
    boot_result = matrix(NA, length(health)*2, ncol(boot_result0))
    boot_result[1, ] = boot_result0[1, ]
    boot_result[3, ] = boot_result0[2, ]
    for(j in 1:ncol(model_combinations)){
      model_set1 = model_combinations[, j]
      boot_result1 = readRDS(paste0(c("MHDResults/MNAR_boot_est_", subset_names[2], "_", model_set1, "_mild_OR13.RDS"), collapse = ""))
      boot_result[2, ] = boot_result1[1, ]
      boot_result[4, ] = boot_result1[2, ]
      
      boot_mu_ipw = apply(boot_result[1:2, ], 1, function(v) mean(rm.extreme(v)))
      boot_var_ipw = apply(boot_result[1:2, ], 1, function(v) var(rm.extreme(v)))
      boot_se_ipw[m] = sqrt(delta_method(boot_mu_ipw[1], boot_mu_ipw[2])^2%*%boot_var_ipw)
...
boot_result[1,] = boot_result0[3, ]
boot_result[2,] = boot_result1[3, ]
boot_mu_ipw = apply(boot_result[1:2, 1:1000], 1, function(v) mean(rm.extreme(v)))
boot_var_ipw = apply(boot_result[1:2, 1:1000], 1, function(v) var(rm.extreme(v)))
boot_se_ipw[20] = sqrt(delta_method(boot_mu_ipw[1], boot_mu_ipw[2])^2%*%boot_var_ipw)
```

```{r}
res = res.tmp
res = cbind(res, boot_se_ipw)
res = as.data.frame(all_results)
res = cbind(round(all_results, 4),
            apply(cbind(all_results[, 1] - qnorm(0.975)*all_results[, 2], all_results[, 1] + qnorm(0.975)*all_results[, 2]), 1, 
...
```{r}
res = all_results[-20, ]
res$estimator <- 1:nrow(all_results)
...
boot.CI = t(apply(boot_result, 1, quantile, c(0.025, 0.975)))[1:7,]
