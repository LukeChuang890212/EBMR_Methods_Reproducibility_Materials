## Perturbation bootstrap for overall analysis (full dataset)
## Same formulas as overall_mean_analysis_Fast4.R
...
cat("=== Perturbation bootstrap for overall analysis ===\n\n")

B <- 1000
n_cores <- min(detectCores() - 1, 10)
cat(sprintf("B=%d, cores=%d, n=%d\n\n", B, n_cores, n))
...
for (j in 1:8) {
  bt1 <- compute_se_capped(perturb_mat[, j])
  bt2 <- compute_se_nocap(perturb_mat[, j])
  n_na <- sum(is.na(perturb_mat[, j]))
