...
```{r}
perturb_bootstrap = function(ps_specifications, data, mu_ipw, B){
  library(foreach)
  library(doParallel)
  library(doSNOW)
  library(EBMRalgorithm)
  ...
  boot_result = foreach(i= 1:B, .combine = 'cbind', .options.snow = opts, .packages = parallel_packages) %dopar% {
    result = tryCatch({
      ...
      result = ebmr$EBMR_IPW_perturb(h_x_names = c("father", "parent_report"), mu_ipw)
      estimates = unlist(result[c(1, 3)])
      
      c(psi_ipw = estimates[1], se_ipw = estimates[2], mu_cc = mean(boot_dat$teacher_report[boot_dat$r == 1]))
    }, error = function(e){
...
for(model_num in 1:3){
  ...
  for(i in 1:ncol(model_combinations)){
    ...
    for(k in 1:length(health)){
      ...
      #  ps_fit.list[[j]] = WangShaoKim2014(formula, h_x_names, inv_link, data = subdat)
      # }
      # 
      # result = EBMR_IPW(h_x_names = c("father", "parent_report", "fp"), data = subdat)
      ...
      # print(ps_fit.list[[4]]$coefficients)
      # print(ps_fit.list[[4]]$se)
      print(result$w.hat)
      # print(cor(result$ps.matrix))

      save_file = paste0(c("MHD_results/EBMR_IPW_", subset_names[k], "_", model_set, "_OR4.RDS"), collapse = "")
...
for(model_num in 1:3){
  ...
  for(i in 1:ncol(model_combinations)){
    ...
    for(k in 1:2) {
      ...
      boot_result = perturb_bootstrap(ps_specifications, subdat, result$mu_ipw, B)
...
for(model_num in 1:3){
  ...
  for(i in 1:ncol(model_combinations)){
    ...
    all_boot_results = matrix(NA, length(health)*2, 500)
    all_boot_results[1, ] = boot_result0[1, 1:500]
    all_boot_results[3, ] = boot_result0[2, 1:500]
    for(j in 1:ncol(model_combinations)){
      model_set1 = model_combinations[, j]
      boot_result1 = readRDS(paste0(c("MHD_results/EBMR_IPW_bootstrap_", subset_names[2], "_", model_set1, "_OR4.RDS"), collapse = ""))
      all_boot_results[2, ] = boot_result1[1, 1:500]
      all_boot_results[4, ] = boot_result1[2, 1:500]
      ...
      boot_mu_ipw = apply(all_boot_results[1:2, ], 1, function(v) mean(rm.extreme(v[!is.na(v)]))) + mu_ipw
