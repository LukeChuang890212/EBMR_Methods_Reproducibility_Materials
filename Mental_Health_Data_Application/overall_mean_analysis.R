...
ps_specifications <- list(
  ...
  outcome = "teacher_report",  # New in EBMRalgorithmFast2: specify outcome variable
...
# Fit EBMR
#------------------------------------------------------------------------------#
ebmr <- EBMRAlgorithmFast2$new("teacher_report", ps_specifications, dat, W)
...
for (j in seq_along(all_model_sets)) {
  ...
  ps_spec_subset <- list(
    ...
    outcome = ps_specifications$outcome,  # Include outcome for EBMRalgorithmFast2
    inv_link = ps_specifications$inv_link
  )

  ebmr_sub <- EBMRAlgorithmFast2$new("teacher_report", ps_spec_subset, dat, W)
  result_sub <- ebmr_sub$EBMR_IPW(h_nu = h_nu, true_ps = NULL)
  ...
  boot_results_raw <- foreach(
    ...
    for (j in seq_along(all_model_sets)) {
      ...
      mu_list[[label]] <- tryCatch({
        ...
        result_b <- ebmr_b$EBMR_IPW(h_nu = h_nu, true_ps = NULL, se.fit = FALSE)
        result_b$mu_ipw
      }, error = function(e) {
        NA
      })
