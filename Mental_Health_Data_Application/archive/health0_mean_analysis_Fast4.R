#------------------------------------------------------------------------------#
# Health=0 Subgroup Mean Estimation: E[teacher_report | health=0]
...
#------------------------------------------------------------------------------#
# Read and prepare data — subset to health=0
#------------------------------------------------------------------------------#
...
# Subset to health=0
dat <- dat_full[dat_full$health == 0, ]
n <- nrow(dat)
...
cat("================================================================================\n")
cat("     HEALTH=0 SUBGROUP MEAN ESTIMATION: E[teacher_report | health=0]           \n")
cat("     Using EBMRalgorithmFast4                                                  \n")
cat("================================================================================\n\n")

cat("Sample size (health=0):", n, "\n")
...
boot_file <- "MHD_results/health0_boot_Fast4_111_B1000.RDS"
...
if (file.exists(boot_file)) {
  ...
  boot_cc <- readRDS("MHD_results/health0_boot_Fast4_CC_B1000.RDS")
  ...
  for (label in model_set_labels) {
    bf <- sprintf("MHD_results/health0_boot_Fast4_%s_B1000.RDS", label)
    if (file.exists(bf)) {
  ...
  boot_results_raw <- foreach(
    ...
    set.seed(12345 + b)
    # Resample from the full dataset, then subset to health=0
    idx <- sample(1:nrow(dat_full), nrow(dat_full), replace = TRUE)
    dat_b_full <- dat_full[idx, ]
    dat_b <- dat_b_full[dat_b_full$health == 0, ]
  ...
  # Save
  saveRDS(boot_cc, "MHD_results/health0_boot_Fast4_CC_B1000.RDS")
  for (label in model_set_labels) {
    saveRDS(boot_results[[label]],
            sprintf("MHD_results/health0_boot_Fast4_%s_B1000.RDS", label))
  }
...
png("MHD_results/health0_sensitivity_analysis_Fast4.png", width = 800, height = 600)
plot(sensitivity_results$xi, sensitivity_results$mu_ipw,
     type = "b", pch = 19, col = "blue",
     xlab = expression(xi),
     ylab = expression(hat(mu)[IPW]),
     main = "Sensitivity Analysis: Health=0 Subgroup Mean (Fast4)\nPerturbing Model 1 with Exponential Tilt",
...
dev.off()
cat("  Saved: MHD_results/health0_sensitivity_analysis_Fast4.png\n")
