...
# Using EBMRalgorithmFast4 (current package)
# Adapted from overall_mean_analysis.R (which used EBMRalgorithmFast2)
#------------------------------------------------------------------------------#
...
# 3 PS models (same as overall_mean_analysis.R / Fast2)
...
boot_file <- "MHD_results/popmean_boot_Fast4_111_B1000.RDS"
...
if (file.exists(boot_file)) {
  ...
  boot_cc <- readRDS("MHD_results/popmean_boot_Fast4_CC_B1000.RDS")
  ...
  for (label in model_set_labels) {
    bf <- sprintf("MHD_results/popmean_boot_Fast4_%s_B1000.RDS", label)
    if (file.exists(bf)) {
  ...
  # Save
  saveRDS(boot_cc, "MHD_results/popmean_boot_Fast4_CC_B1000.RDS")
  for (label in model_set_labels) {
    saveRDS(boot_results[[label]],
            sprintf("MHD_results/popmean_boot_Fast4_%s_B1000.RDS", label))
  }
...
compute_trimmed_boot_se <- function(x, label) {
  ...
  # Same logic as clean_sim_result: IQR x 3, cap at 1%
  ...
  list(se_raw = sd(x), se_trim = sd(x_trim), n_na = n_na, n_outlier = n_outlier)
}

#------------------------------------------------------------------------------#
# Final Summary (with and without outlier removal)
#------------------------------------------------------------------------------#
cat("\n================================================================================\n")
cat("                              FINAL SUMMARY                                     \n")
cat("================================================================================\n\n")

# Compute trimmed bootstrap SE
...
png("MHD_results/popmean_sensitivity_analysis_Fast4.png", width = 800, height = 600)
plot(sensitivity_results$xi, sensitivity_results$mu_ipw,
     type = "b", pch = 19, col = "blue",
     xlab = expression(xi),
     ylab = expression(hat(mu)[IPW]),
     main = "Sensitivity Analysis: Population Mean Estimation (Fast4)\nPerturbing Model 1 with Exponential Tilt",
...
dev.off()
cat("  Saved: MHD_results/popmean_sensitivity_analysis_Fast4.png\n")
