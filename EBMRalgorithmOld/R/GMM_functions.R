...
gmm = function(g, W, n, esteq_dim, param_dim, init, se.fit = T){
  G = function(param){
    g.matrix = g(param)
    return(matrix(apply(g.matrix, 2, mean), esteq_dim, 1))
  }
  ...
  while (conv_err > 10^(-20) & t < 1000){
    # obj1 = function(par) 3^(obj(par)+1)
    opt = nlminb(sol_path[, t], obj,
