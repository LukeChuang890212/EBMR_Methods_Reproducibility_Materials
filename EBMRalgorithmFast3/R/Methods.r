...
WangShaoKim2014 = function(formula, outcome = NULL, h_alpha, inv_link,
                           init = NULL, se.fit = TRUE, wt = NULL, method = "GN") {
  ...
  if (is.mnar) {
    ...
    gmm_fit = private$gmm(Phi_alpha, private$W, n, h_dim, alpha_dim, init, se.fit, dg_alpha, d2g_alpha, method = method)
...
ensemble = function(ps.matrix, h_nu, init = NULL, se.fit = T, wt = NULL, method = "GN") {
  # Basic setup
  ...
  gmm_fit = private$gmm(Phi_nu, private$W, n, h_dim, J, init, se.fit, dg_nu, d2g_nu, method = method)
...
EBMR_IPW = function(h_nu, model_indices = NULL, nu_init = rep(1/J, J), se.fit = TRUE, true_ps = NULL, wt = NULL, type = c("HT", "Hajek"), method = "GN") {
  # Basic setup
  r = as.matrix(private$r)
  ...
  # if (is.null(nu_init)) nu_init = c(0.9999, rep(0, J-1))
  ...
  # Ensemble step
  #-----------------------------------------------------------------------------#
  ensemble_fit = private$ensemble(ps.matrix, h_nu, nu_init, se.fit, method = method)
...
EBMR_IPW_with_locally_misspecified_model = function(ps.matrix, perturb_ps, exp_tilt, exp_tilt_x_names, h_nu, nu_init = rep(1/J, J), se.fit = FALSE, type = c("HT", "Hajek"), method = "GN"){
  # Basic setup
  r = as.matrix(private$r)
  y = as.matrix(private$y)
  n = private$n
...
EBMR_IPW_regression = function(h_nu, reg_formula, family = "gaussian",
                                ...
                                se.fit = TRUE, true_ps = NULL, wt = NULL, method = "GN") {
  # Basic setup
  r = as.matrix(private$r)
  y = as.matrix(private$y)
  n = private$n
