...
setting12.B1 = function(n){
  ...
  m = function(z1, z2, u1, u2) 0.2+z1+z2+0.5*u1+0.5*u2
  response.prob = function(y, u1, u2){
    propensity =   1/(1+exp(1.2+0.4*y-2*u1-0.5*u2))*exp(n^(-1/2)*(-y+u1-u2))
    propensity[propensity > 1] = 0.95
    return(propensity)
  }
  eta = m(z1, z2, u1, u2)
...
setting14.A1 = function(n, response.rate){
  z1 = rbinom(n, size = 1, prob = 0.3)
  z2 = rnorm(n, mean = 0, sd = 1)
...
setting14.B2 = function(n, response.rate){
  z1 = rbinom(n, size = 1, prob = 0.3)
  z2 = rnorm(n, mean = 0, sd = 1)
