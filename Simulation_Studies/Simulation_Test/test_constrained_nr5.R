...
run_variant <- function(cond_thresh, trust_radius) {
  ...
  cat(sprintf("cond<%.0e, %s: ESE2/ESD=%.3f, Bias=%.4f, alpha2 range=[%.2f, %.2f], conv=%d/%d\n",
      cond_thresh, tr_label, ese2/esd, mean(mu_vals[valid]) - mu_true,
      min(alpha2_vals[valid]), max(alpha2_vals[valid]), sum(conv_vals[valid]), sum(valid)))
}

cat("=== Higher cond thresholds with trust=2.0 ===\n")
for (ct in c(5e7, 1e8, 2e8, 5e8, 1e9)) {
  run_variant(ct, 2.0)
}

cat("\n=== Fine-tune trust radius around 2.0 with cond=5e7 ===\n")
for (tr in c(1.5, 1.75, 2.0, 2.25, 2.5)) {
  run_variant(5e7, tr)
}

cat("\n=== Fine-tune trust radius around 2.0 with cond=1e8 ===\n")
for (tr in c(1.5, 1.75, 2.0, 2.25, 2.5)) {
  run_variant(1e8, tr)
}
