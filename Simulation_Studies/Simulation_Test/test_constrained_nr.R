...
run_test <- function(setting, miss, ps_spec_id, model_idx, cond_threshold = 1e6) {
  ...
  for (rep_i in 1:n_reps) {
    ...
    tryCatch({
      ...
      # Custom NR inner optimizer: fully minimize Q(alpha, W) for fixed W
      ...
      nr_inner <- function(start, W_hat, max_iter = 500L) {
        ...
        for (k in 1:max_iter) {
          ...
          # Backtracking line search with degeneracy constraint
          ...
          for (ls in 1:30) {
            ...
            if (is.finite(obj_new) && obj_new < obj_cur - 1e-4 * step * sum(g_vec * direction)) {
              # Degeneracy check: is the candidate in the flat region?
              ...
              # Armijo satisfied but degenerate — reduce step to stay in feasible region
            }
            step <- step * 0.5
          }
          ...
          if (accepted) {
            ...
            # Cannot make progress without entering degenerate region
            # This means we've found the boundary of the feasible region
            break
          }
        }
      ...
      # === Standard iterated GMM with custom NR inner optimizer ===
