...
run_leashed_gmm <- function(dat, design_mat, h_x, link_type) {
  ...
  dist_final <- sqrt(sum((best_est - init)^2))
  list(estimates = best_est, grad_norm = final_grad, cond = final_cond,
       pi_hat = pi_final, dist = dist_final)
}
...
mu_vals <- se_vals <- cond_vals <- grad_vals <- dist_vals <- rep(NA_real_, n_reps)
for (rep_i in 1:n_reps) {
...
conv <- valid & grad_vals < tol
nonconv <- valid & grad_vals >= tol

cat(sprintf("\n=== dist_max=6, 1000 reps: Diagnostics ===\n"))
cat(sprintf("  Converged: %d, Non-converged: %d\n", sum(conv), sum(nonconv)))

cat("\n--- Converged reps ---\n")
cat(sprintf("  Bias     = %.4f\n", mean(mu_vals[conv]) - mu_true))
cat(sprintf("  ESD      = %.4f\n", sd(mu_vals[conv])))
cat(sprintf("  ESE      = %.4f\n", mean(se_vals[conv])))
cat(sprintf("  ESE/ESD  = %.3f\n", mean(se_vals[conv]) / sd(mu_vals[conv])))
ci_lo <- mu_vals[conv] - 1.96 * se_vals[conv]
ci_hi <- mu_vals[conv] + 1.96 * se_vals[conv]
cat(sprintf("  CP       = %.3f\n", mean(mu_true >= ci_lo & mu_true <= ci_hi)))

cat("\n--- Non-converged reps ---\n")
cat(sprintf("  Bias     = %.4f\n", mean(mu_vals[nonconv]) - mu_true))
cat(sprintf("  ESD      = %.4f\n", sd(mu_vals[nonconv])))
cat(sprintf("  ESE      = %.4f\n", mean(se_vals[nonconv])))
cat(sprintf("  ESE/ESD  = %.3f\n", mean(se_vals[nonconv]) / sd(mu_vals[nonconv])))
ci_lo2 <- mu_vals[nonconv] - 1.96 * se_vals[nonconv]
ci_hi2 <- mu_vals[nonconv] + 1.96 * se_vals[nonconv]
cat(sprintf("  CP       = %.3f\n", mean(mu_true >= ci_lo2 & mu_true <= ci_hi2)))

cat("\n--- Gradient distribution (non-converged) ---\n")
g_nc <- grad_vals[nonconv]
cat(sprintf("  min=%.2e, Q25=%.2e, median=%.2e, Q75=%.2e, max=%.2e\n",
    min(g_nc), quantile(g_nc, 0.25), median(g_nc), quantile(g_nc, 0.75), max(g_nc)))

cat("\n--- Distance from init ---\n")
cat(sprintf("  Converged:     min=%.2f, median=%.2f, max=%.2f\n",
    min(dist_vals[conv]), median(dist_vals[conv]), max(dist_vals[conv])))
cat(sprintf("  Non-converged: min=%.2f, median=%.2f, max=%.2f\n",
    min(dist_vals[nonconv]), median(dist_vals[nonconv]), max(dist_vals[nonconv])))

cat("\n--- Cond distribution ---\n")
cat(sprintf("  Converged:     min=%.2e, median=%.2e, max=%.2e\n",
    min(cond_vals[conv]), median(cond_vals[conv]), max(cond_vals[conv])))
cat(sprintf("  Non-converged: min=%.2e, median=%.2e, max=%.2e\n",
    min(cond_vals[nonconv]), median(cond_vals[nonconv]), max(cond_vals[nonconv])))

# How many non-converged are at the leash boundary?
at_boundary <- nonconv & dist_vals > (dist_max - 0.1)
cat(sprintf("\n  Non-converged at leash boundary (dist>5.9): %d / %d\n",
    sum(at_boundary), sum(nonconv)))
