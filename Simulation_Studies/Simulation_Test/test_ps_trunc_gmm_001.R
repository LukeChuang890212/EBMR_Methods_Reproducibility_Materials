...
cat("Running 1000 reps with INSIDE-GMM truncation at 0.01, no outside truncation...\n")
n_err <- 0
for (rep_i in 1:n_reps) {
  ...
  tryCatch({
    ...
    n_truncated[rep_i] <- sum(ps_vec <= 0.01 + 1e-10)
...
cat(sprintf("\n=== TRUNCATION SUMMARY (ps at floor 0.01) ===\n"))
