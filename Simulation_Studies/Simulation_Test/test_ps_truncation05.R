...
cat("Running 1000 reps with ps truncation at 0.05...\n")
n_err <- 0
for (rep_i in 1:n_reps) {
  ...
  tryCatch({
    ...
    # Count how many obs had ps < 0.05 before truncation
    ps_raw <- res$ps.matrix[, 1]
    n_truncated[rep_i] <- sum(ps_raw < 0.05)
  }, error = function(e) {
    n_err <<- n_err + 1
