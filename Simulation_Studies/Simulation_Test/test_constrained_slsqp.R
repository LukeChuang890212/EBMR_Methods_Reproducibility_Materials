...
## Uses nloptr::slsqp (Sequential Least Squares QP) for proper constrained optimization
...
run_constrained_gmm <- function(dat, design_mat, h_x, link_type) {
  ...
  # Jacobian of the constraint w.r.t. alpha
  constraint_jac <- function(alpha) {
    pi_v <- compute_pi_fn(as.vector(design_mat %*% alpha))
    # d/dalpha E[r/pi - 1] = E[r/pi^2 * dpi/dalpha]
    #   = E[r * (1-pi)/pi * design_mat] / n  (for logistic complement link)
    as.vector(colMeans(r_vec * (1 - pi_v) / pi_v * design_mat))
  }

  # --- SLSQP-based constrained optimization for fixed W ---
  ...
  list(estimates = estimates, grad_norm = final_grad_norm, cond = final_cond,
       pi_hat = pi_final, constraint_viol = constraint_viol)
}
...
cat("=== Constrained GMM via SLSQP: E[r/pi-1]=0 ===\n\n")
