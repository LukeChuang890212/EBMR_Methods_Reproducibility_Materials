...
# Leashed inner + prefer-local:
# 1. Full inner NR with distance leash from init
# 2. Track best local solution (within local_dist of init)
# 3. If final solution is at boundary and didn't converge, use best local
run_leashed_local_gmm <- function(dat, design_mat, h_x, link_type,
                                   dist_max = 6, local_dist = 4,
                                   local_grad_th = 0.1, trust_r = 2.0) {
  r_vec <- dat[["r"]]
  ...
  for (t in 1:100) {
    ...
    d <- sqrt(sum((est - init)^2))
    if (d <= local_dist && grad < best_local_grad) {
      best_local_grad <- grad; best_local_est <- est
    }
  ...
  # Decision: if best solution didn't converge and is at boundary, prefer local
  final_est <- best_est
  used_local <- FALSE
  converged <- (best_grad < tol)
  at_boundary <- sqrt(sum((best_est - init)^2)) > (dist_max - 0.1)

  if (!converged && at_boundary && best_local_grad < local_grad_th) {
  ...
  dist_final <- sqrt(sum((final_est - init)^2))

  list(estimates = final_est, grad_norm = final_grad, cond = final_cond,
       pi_hat = pi_final, dist = dist_final, used_local = used_local,
       converged = converged, at_boundary = at_boundary,
       best_local_grad = best_local_grad)
}
...
# Test configurations: (dist_max, local_dist, local_grad_th)
configs <- list(
  list(dm=6, ld=3, gt=0.1,  lab="dm=6, ld=3, gt=0.1"),
  list(dm=6, ld=4, gt=0.1,  lab="dm=6, ld=4, gt=0.1"),
  list(dm=6, ld=4, gt=0.05, lab="dm=6, ld=4, gt=0.05"),
  list(dm=7, ld=4, gt=0.1,  lab="dm=7, ld=4, gt=0.1"),
  list(dm=7, ld=5, gt=0.1,  lab="dm=7, ld=5, gt=0.1")
)

for (cfg in configs) {
  cat(sprintf("\n=== Leashed+local: %s ===\n", cfg$lab))
  mu_vals <- se_vals <- cond_vals <- grad_vals <- rep(NA_real_, n_reps)
  n_used_local <- 0; n_at_boundary <- 0; n_conv <- 0
  ...
  for (rep_i in 1:n_reps) {
    ...
    tryCatch({
      res <- run_leashed_local_gmm(dat, di$design_mat, di$h_x, di$link_type,
                                    dist_max = cfg$dm, local_dist = cfg$ld,
                                    local_grad_th = cfg$gt)
      cond_vals[rep_i] <- res$cond
      grad_vals[rep_i] <- res$grad_norm
      if (res$used_local) n_used_local <- n_used_local + 1
      if (res$at_boundary) n_at_boundary <- n_at_boundary + 1
      if (res$converged) n_conv <- n_conv + 1
  ...
  cat(sprintf("  Valid: %d, Degen: %d, Conv: %d, AtBound: %d, UsedLocal: %d\n",
      sum(valid), n_degen, n_conv, n_at_boundary, n_used_local))
