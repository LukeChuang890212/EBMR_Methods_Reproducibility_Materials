...
# Pre-build all design matrices to avoid repeated EBMRAlgorithmFast4$new calls
...
for (rep_i in 1:n_reps) {
  ...
  tryCatch({
    ...
    cat(sprintf("  Rep %d prebuild ERROR: %s\n", rep_i, e$message))
  })
}
cat("Done.\n")

for (dm in c(3, 4, 5, 6, 7, 8, 10)) {
  cat(sprintf("\n=== dist_max = %d, 1000 reps ===\n", dm))
  ...
  for (rep_i in 1:n_reps) {
    ...
    tryCatch({
      res <- run_leashed_gmm(dat, di$design_mat, di$h_x, di$link_type, dist_max = dm)
