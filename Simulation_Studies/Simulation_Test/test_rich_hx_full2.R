...
## Reports results WITH and WITHOUT outlier removal (clean_sim_result)
...
build_h_rich <- function(dat) {
  h <- cbind(1, dat$u1, dat$u2, dat$z1, dat$z2,
             dat$u1*dat$u2, dat$u1*dat$z1, dat$u1*dat$z2,
             dat$u2*dat$z1, dat$u2*dat$z2, dat$z1*dat$z2)
  colnames(h) <- c("1","u1","u2","z1","z2",
                    "u1u2","u1z1","u1z2","u2z1","u2z2","z1z2")
  h
}

## Summary function: reports with and without outlier removal
report_summary <- function(mu_vals, se_vals, cond_vals, mu_true, label) {
  ...
  cat(sprintf("  --- %s (all valid, n=%d, degen=%d) ---\n", label, n_valid, n_degen))
  cat(sprintf("  Bias     = %.4f\n", mean(mu_v) - mu_true))
  cat(sprintf("  ESD      = %.4f\n", sd(mu_v)))
  cat(sprintf("  ESE      = %.4f\n", mean(se_v)))
...
cat("=== Full simulation: Enriched h_x (11 instruments, overid=7) ===\n\n")
...
for (cfg in settings) {
  ...
  report_summary(mu_vals, se_vals, cond_vals, mu_true, cfg$name)
}
