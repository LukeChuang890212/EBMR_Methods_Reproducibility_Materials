...
for (rep_i in 1:n_reps) {
  ...
  tryCatch({
    ...
    # NR inner minimizer: fully minimize Q(alpha, W) for fixed W
    # with trust region to prevent alpha going extreme
    nr_inner <- function(start, W_hat, max_inner = 200L) {
      alpha <- start
      init_inner <- start  # track distance from inner start
      best_inner_grad <- Inf
      best_inner <- alpha
    ...
    # Standard iterative GMM with NR inner loop
    # Step 1: W = I
