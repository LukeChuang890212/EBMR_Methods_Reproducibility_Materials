...
hessian_min_eig <- numeric(n_reps)
...
for (i in 1:n_reps) {
  ...
  tryCatch({
    ...
    hessian_pd[i] <- gmm_fit$opt$hessian_pd
    hessian_min_eig[i] <- gmm_fit$opt$hessian_min_eig
...
valid <- !is.na(mu_hat)
cat(sprintf("\n=== BOUNDS=5 + Hessian PD constraint: %d/%d valid reps ===\n", sum(valid), n_reps))
...
# Hessian PD
cat(sprintf("\n=== HESSIAN PD ===\n"))
cat(sprintf("  Overall PD: %d/%d (%.0f%%)\n", sum(hessian_pd[valid]), sum(valid), 100*mean(hessian_pd[valid])))
if (sum(is_outlier) > 0) {
  cat(sprintf("  Outlier PD:     %d/%d\n", sum(hessian_pd[outlier_orig_idx]), length(outlier_orig_idx)))
}
cat(sprintf("  Non-outlier PD: %d/%d (%.0f%%)\n", sum(hessian_pd[normal_orig_idx]), length(normal_orig_idx), 100*mean(hessian_pd[normal_orig_idx])))

cat(sprintf("\n  min_eig (Hessian):\n"))
if (sum(is_outlier) > 0) {
  cat(sprintf("    Outlier:     mean=%.4f, min=%.4f, max=%.4f\n",
              mean(hessian_min_eig[outlier_orig_idx]), min(hessian_min_eig[outlier_orig_idx]), max(hessian_min_eig[outlier_orig_idx])))
}
cat(sprintf("    Non-outlier: mean=%.4f, min=%.4f, max=%.4f\n",
            mean(hessian_min_eig[normal_orig_idx]), min(hessian_min_eig[normal_orig_idx]), max(hessian_min_eig[normal_orig_idx])))

# Iterations
cat(sprintf("\n=== ITERATIONS ===\n"))
if (sum(is_outlier) > 0) {
  cat(sprintf("  Outlier:     mean=%.1f, range=[%d, %d]\n",
              mean(iterations[outlier_orig_idx]), min(iterations[outlier_orig_idx]), max(iterations[outlier_orig_idx])))
}
cat(sprintf("  Non-outlier: mean=%.1f, range=[%d, %d]\n",
...
cat(sprintf("  Cor(hessian_min_eig, |mu-median|) = %.3f\n", cor(hessian_min_eig[valid], abs(mu_v - median(mu_v)))))
...
if (sum(is_outlier) > 0) {
  ...
  for (ii in seq_along(outlier_orig_idx)) {
    ...
    cat(sprintf("  Rep %3d: mu=%8.4f, obj=%.6f, conv=%s, iter=%d, hPD=%s, min_eig=%.4f, min_ps=%.6f, max_ipw=%6.1f, alpha=[%s]\n",
                k, mu_hat[k], objective[k], converged[k], iterations[k],
                hessian_pd[k], hessian_min_eig[k],
                min_ps[k], max_ipw[k],
