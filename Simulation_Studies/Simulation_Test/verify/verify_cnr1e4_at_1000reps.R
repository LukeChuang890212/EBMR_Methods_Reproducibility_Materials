## Head-to-head verification at 1000 reps: does cnr/1e4 actually improve CP from
## the user's saved 0.928 (default) to ~0.95? Use the same 1000-rep slice.
...
report <- function(tag, out) {
  ...
  cat(sprintf("%-28s | Bias=%+.4f  ESD=%.4f  ESE=%.4f  ratio=%.3f  CP=%.3f | max|a| q99=%.2f max=%.2f | blowups=%d (%.0fs)\n",
              tag, bias, esd, ese, ese/esd, cp,
              quantile(ma_v, 0.99, na.rm=T), max(ma_v, na.rm=T), n_blow, out$elapsed))
}

cat("===== 1000-rep head-to-head =====\n")
o1 <- run_one("L-BFGS-B", 1e8, "default L-BFGS-B/1e8")
report("default L-BFGS-B/1e8", o1)
o2 <- run_one("constrained_nr", 1e4, "cnr/1e4")
report("cnr/1e4", o2)
o3 <- run_one("constrained_nr", 1e5, "cnr/1e5")
report("cnr/1e5", o3)
