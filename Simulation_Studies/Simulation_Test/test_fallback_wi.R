## Approach C: Standard constrained_nr + fallback to W=I solution when degenerate
## Run full constrained_nr as in package. If final solution is degenerate
## (cond > threshold), use the W=I intermediate solution instead.
...
run_cnr_fallback_wi <- function(dat, design_mat, h_x, link_type) {
  r_vec <- dat[["r"]]
  ...
  # Step 1: W = I
  wi_solution <- nr_inner(init, diag(h_dim))

  # Step 2: Full iterative GMM with stall detection + restarts
  estimates <- wi_solution
  ...
  # L-BFGS-B fallback (standard, no cond guard)
  ...
  if (final_grad_norm >= tol) {
    cnr_estimates <- estimates; cnr_grad_norm <- final_grad_norm
    run_lbfgsb <- function(start_point) {
      st <- new.env(parent = emptyenv())
      st$W.hat <- NULL; st$cache_param <- NULL; st$cache_g <- NULL; st$cache_G <- NULL
      cache_fn <- function(param) {
        if (!is.null(st$cache_param) && identical(param, st$cache_param)) return()
        st$cache_param <- param; st$cache_g <- g_fn(param)
        st$cache_G <- matrix(.colMeans(st$cache_g, n, h_dim), h_dim, 1)
      }
      obj_fn <- function(param) {
        cache_fn(param); v <- as.numeric(crossprod(st$cache_G, st$W.hat %*% st$cache_G))
        if (!is.finite(v)) 1e8 else v
      }
      grad_fn <- function(param) {
        cache_fn(param); 2 * as.vector(crossprod(Gamma_fn(param), st$W.hat %*% st$cache_G))
      }
      est <- start_point; st$W.hat <- diag(h_dim)
      opt0 <- optim(est, obj_fn, gr = grad_fn, method = "L-BFGS-B", control = list(maxit = 1000))
      est <- opt0$par
      for (t_fb in 1:max_outer) {
        cache_fn(est); st$W.hat <- tryCatch(W_fn(st$cache_g), error = function(e) diag(h_dim))
        fg <- max(abs(grad_fn(est))); if (fg < tol) break
        st$cache_param <- NULL
        opt_t <- optim(est, obj_fn, gr = grad_fn, method = "L-BFGS-B", control = list(maxit = 1000))
        est <- opt_t$par
      }
      g_f <- g_fn(est); G_f <- matrix(.colMeans(g_f, n, h_dim), h_dim, 1)
      W_f <- tryCatch(W_fn(g_f), error = function(e) diag(h_dim))
      list(estimates = est, grad_norm = max(abs(2 * as.vector(crossprod(Gamma_fn(est), W_f %*% G_f)))))
    }
    fb1 <- run_lbfgsb(cnr_estimates); fb2 <- run_lbfgsb(init)
    fb <- if (fb1$grad_norm <= fb2$grad_norm) fb1 else fb2
    if (fb$grad_norm < cnr_grad_norm) {
      estimates <- fb$estimates; final_grad_norm <- fb$grad_norm; used_fallback <- TRUE
    }
  }

  # KEY: If final solution is degenerate, fall back to W=I solution
  final_cond <- forward_cond(estimates)
  used_wi_fallback <- FALSE
  if (final_cond >= cond_limit) {
    wi_cond <- forward_cond(wi_solution)
    if (wi_cond < cond_limit) {
      estimates <- wi_solution
      final_cond <- wi_cond
      g_mat_f2 <- g_fn(estimates)
      G_f2 <- matrix(.colMeans(g_mat_f2, n, h_dim), h_dim, 1)
      W_f2 <- tryCatch(W_fn(g_mat_f2), error = function(e) diag(h_dim))
      final_grad_norm <- max(abs(2 * as.vector(crossprod(Gamma_fn(estimates), W_f2 %*% G_f2))))
      used_wi_fallback <- TRUE
    }
  }

  pi_final <- compute_pi_fn(as.vector(design_mat %*% estimates))
  list(estimates = estimates, grad_norm = final_grad_norm, cond = final_cond,
       pi_hat = pi_final, used_fallback = used_fallback, used_wi_fallback = used_wi_fallback)
}
...
cat("=== Approach C: Constrained_nr + fallback to W=I when degenerate ===\n\n")
...
for (cfg in settings) {
  ...
  for (rep_i in 1:n_reps) {
    ...
    tryCatch({
      res <- run_cnr_fallback_wi(dat, di$design_mat, di$h_x, di$link_type)
      cond_vals[rep_i] <- res$cond
      grad_vals[rep_i] <- res$grad_norm
      if (res$used_fallback) n_fb <- n_fb + 1
      if (res$used_wi_fallback) n_wi_fb <- n_wi_fb + 1
  ...
  cat(sprintf("  L-BFGS-B fallback: %d, W=I fallback: %d\n", n_fb, n_wi_fb))
