...
# Original h_alpha: full = c("u1", "u2", "z1", "z2")
# Enriched h_alpha: full_sq1 = function(dat) cbind(u1, u2, z1, z2, u2^2)
h_alpha_enriched <- function(dat) {
  cbind(u1 = dat$u1, u2 = dat$u2, z1 = dat$z1, z2 = dat$z2, u2_sq = dat$u2^2)
}

# ===== Test 1: Stubborn reps only with enriched h_alpha =====
stubborn_reps <- c(35, 38, 143, 155, 156, 190)
cat("=== Stubborn reps with enriched h_alpha (full_sq1) ===\n")
for (rep_i in stubborn_reps) {
  dat <- all_data[((rep_i-1)*n_val + 1):(rep_i*n_val), ]
  single_ps <- list(
    formula.list = list(ps_spec_orig[["formula.list"]][[m_idx]]),
    h_alpha.list = list(h_alpha_enriched),
...
# ===== Test 2: All 200 reps with enriched h_alpha =====
cat("\n=== All 200 reps: M3 with enriched h_alpha (full_sq1) ===\n")
...
for (rep_i in 1:n_reps) {
  ...
  tryCatch({
    ...
    # Use psi2 from the package for SE2
