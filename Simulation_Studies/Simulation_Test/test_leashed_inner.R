...
# Distance-leashed inner loop:
# Full inner NR, but reject any candidate that moves beyond dist_max from init
run_leashed_gmm <- function(dat, design_mat, h_x, link_type,
                             dist_max = 5, trust_r = 2.0, max_outer = 100) {
  r_vec <- dat[["r"]]
  ...
  # Full inner NR with distance leash from init
  nr_inner <- function(start, W_hat) {
    ...
    for (iter in 1:500) {
      ...
      for (ls in 1:30) {
        ...
        # LEASH: reject if too far from init
        if (sqrt(sum((cand - init)^2)) > dist_max) {
          step <- step * 0.5
          next
        }
        g_cand <- g_fn(cand)
...
# ===== Test multiple dist_max values =====
for (dm in c(3, 5, 7)) {
  cat(sprintf("\n=== Leashed inner: dist_max = %d ===\n", dm))
  ...
  for (rep_i in 1:n_reps) {
    ...
    tryCatch({
      ...
      res <- run_leashed_gmm(dat, design_mat, h_x, link_type, dist_max = dm)
