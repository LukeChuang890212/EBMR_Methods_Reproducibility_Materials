...
# One NR step per W update + trust region + cond(H) guard
# The cond(H) guard replaces the ad-hoc "distance < 5" heuristic
# with a principled check: reject iterates where Gamma'WGamma is degenerate
run_test <- function(setting, miss, ps_spec_id, model_idx, cond_threshold = 1e6) {
  ...
  for (rep_i in 1:n_reps) {
    ...
    tryCatch({
      ...
      # One NR step per W update
      estimates <- rep(0, p)
      best_grad <- Inf
      best_est <- estimates
      # Track best iterate with non-degenerate Hessian
      best_nondeg_grad <- Inf
      best_nondeg_est <- estimates
      no_improve <- 0L
      converged <- FALSE
      ...
      for (t in 1:1000) {
        ...
        # Check cond(H) at current iterate
        ...
        # Track best non-degenerate iterate
        if (cond_H < cond_threshold && grad_norm < best_nondeg_grad) {
          best_nondeg_grad <- grad_norm
          best_nondeg_est <- estimates
        }
        ...
        if (grad_norm < best_grad) {
          ...
          if (no_improve >= 20L) {
            # On stall: prefer non-degenerate best
            estimates <- if (best_nondeg_grad < Inf) best_nondeg_est else best_est
            break
          }
        }
        ...
        # Backtracking line search with cond(H) check
        ...
        for (ls in 1:20) {
          ...
          if (is.finite(obj_new) && obj_new < obj_cur - 1e-4 * step * sum(g_vec * direction)) {
            # Check: would this step enter degenerate region?
        ...
        # If not accepted: stay at current iterate, W will update, try again
      }

      # Final: if ended degenerate, use best non-degenerate
      Gamma_final <- Gamma_fn(estimates)
      H_final <- crossprod(Gamma_final, W_hat %*% Gamma_final)
      eig_final <- eigen(H_final, symmetric = TRUE, only.values = TRUE)$values
      cond_final <- max(eig_final) / max(min(eig_final), 1e-15)
      if (cond_final > cond_threshold && best_nondeg_grad < Inf) {
        estimates <- best_nondeg_est
      }
