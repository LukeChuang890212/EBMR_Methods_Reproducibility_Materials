...
cat("Running 1000 reps with iterative GMM, tol=1e-8...\n")
n_err <- 0
...
bias_raw <- mean(mu_raw) - mu.true
esd_raw <- sd(mu_raw)
ese_raw <- mean(se_raw)
ci_l <- mu_raw - 1.96 * se_raw
ci_u <- mu_raw + 1.96 * se_raw
cp_raw <- mean(mu.true >= ci_l & mu.true <= ci_u)

cat(sprintf("\nBefore outlier removal (n=%d):\n", length(mu_raw)))
cat(sprintf("  Bias = %.4f, ESD = %.4f, ESE = %.4f, ESE/ESD = %.2f, CP = %.4f\n",
            bias_raw, esd_raw, ese_raw, ese_raw/esd_raw, cp_raw))
