...
cat("=== CONV GRAD TEST: 2*Gamma(a)'W(a)G(a) convergence ===\n\n")
for (rep_i in test_reps) {
  ...
  tryCatch({
    ...
    for (m in 1:2) {
      ...
      cat(sprintf("  Model %d: ||a||=%.2f conv=%s iter=%d grad=%.2e obj=%.6f\n",
    ...
    cat(sprintf("  Ensemble: w=(%.4f,%.4f) conv=%s iter=%d grad=%.2e obj=%.6f\n",
