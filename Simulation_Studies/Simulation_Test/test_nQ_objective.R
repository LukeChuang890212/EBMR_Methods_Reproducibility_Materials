...
# GMM with nQ(alpha) = n * G'WG as objective
run_nQ_gmm <- function(dat, design_mat, h_x, link_type) {
  r_vec <- dat[["r"]]
  ...
  obj_fn <- function(alpha, W_hat) {
    ...
    n * as.numeric(crossprod(G, W_hat %*% G))
  }
  # grad of nQ = 2n * Gamma'W G
  grad_fn <- function(alpha, W_hat) {
    g_mat <- g_fn(alpha)
    G <- matrix(.colMeans(g_mat, n, h_dim), h_dim, 1)
    Gamma <- Gamma_fn(alpha)
    2 * n * as.vector(crossprod(Gamma, W_hat %*% G))
  }
  ...
  nr_inner <- function(start, W_hat, trust_r = 2.0) {
    ...
    for (iter in 1:500) {
      ...
      Gamma <- Gamma_fn(alpha)
      # Gradient of nQ
      gv <- 2 * n * as.vector(crossprod(Gamma, W_hat %*% G_vec))
      if (max(abs(gv)) < tol) break
      # Hessian of nQ: 2n * Gamma'W Gamma (Gauss-Newton approx)
      H_gn <- 2 * n * crossprod(Gamma, W_hat %*% Gamma)
      ...
      # Line search on nQ
      obj_cur <- n * as.numeric(crossprod(G_vec, W_hat %*% G_vec))
      accepted <- FALSE; step <- 1.0
      for (ls in 1:30) {
        cand <- alpha + step * direction
        g_cand <- g_fn(cand)
        G_cand <- matrix(.colMeans(g_cand, n, h_dim), h_dim, 1)
        obj_new <- n * as.numeric(crossprod(G_cand, W_hat %*% G_cand))
  ...
  for (t in 1:100) {
    ...
    Gamma <- Gamma_fn(est)
    G_vec <- matrix(.colMeans(g_mat, n, h_dim), h_dim, 1)
    grad <- max(abs(2 * n * as.vector(crossprod(Gamma, W_hat %*% G_vec))))
  ...
  final_grad <- max(abs(grad_fn(best_est, W_final)))
  list(estimates = best_est, grad_norm = final_grad, cond = final_cond)
}

# SE computation (CUE SE)
...
# ===== Test: stubborn reps first =====
stubborn_reps <- c(35, 38, 143, 155, 156, 190)
m_idx <- 3

cat("=== Stubborn reps: nQ objective ===\n")
for (rep_i in stubborn_reps) {
  ...
  tryCatch({
    ...
    res <- run_nQ_gmm(dat, design_mat, h_x, link_type)
    ...
    cat(sprintf("  Rep %3d: grad=%.2e, cond=%.2e, mu=%.4f, alpha[2]=%.3f, #pi>0.99=%d\n",
        rep_i, res$grad_norm, res$cond, se_res$mu, res$estimates[2],
        sum(compute_pi_fn(as.vector(design_mat %*% res$estimates)) > 0.99)))
  }, error = function(e) {
...
cat(sprintf("\n  Valid: %d/%d, Degenerate: %d, Converged (grad<tol): %d\n",
