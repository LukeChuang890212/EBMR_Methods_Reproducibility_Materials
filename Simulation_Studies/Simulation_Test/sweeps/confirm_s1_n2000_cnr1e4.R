## Re-confirm s1.B1 n=2000, M3 only, constrained_nr cond=1e4, 1000 reps.
...
cat(sprintf("\nConfig: s1.B1 n=%d, M3 only, cnr/cond=1e4, %d reps  (mu_true=%.4f)\n",
            nn, n_reps, mu_true))
...
saveRDS(list(res = res, ps_spec = ps_spec, n_reps = n_reps, nn = nn,
        ...
        "Simulation_Test/confirm_s1_n2000_cnr1e4_results.RDS")
cat("\nDONE\n")
