## Confirm s1.B1 n=2000, M2 only, constrained_nr cond=1e3, 1000 reps
## (matches test65 file EBMR_IPW_setting1-miss50-scenario9-3_2_n2000_replicate1000_test65.RDS)
...
cat(sprintf("\nConfig: s1.B1 n=%d, M2 only, cnr/cond=1e3, %d reps  (mu_true=%.4f)\n",
            nn, n_reps, mu_true))
...
cat(sprintf("\nNA=%d Out=%d (clean_sim_result)  stuck@0=%d sep(>50)=%d\n",
...
cat(sprintf("\nelapsed: %.0fs\n", elapsed))
saveRDS(list(res = res, cleaned = cleaned), "Simulation_Test/confirm_s1_n2000_M2_cnr1e3_results.RDS")
cat("\nDONE\n")
