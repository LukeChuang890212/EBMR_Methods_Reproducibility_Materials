## Confirm s2.B1 n=2000, M3 only, constrained_nr cond=1e4, Fast4, 1000 reps
...
cat(sprintf("\nConfig: s2.B1 n=%d, M3 only, cnr/cond=1e4 (Fast4), %d reps  mu_true(data)=%.4f\n",
            nn, n_reps, mu_true))
...
cat(sprintf("median(alpha)=(%.3f, %.3f, %.3f, %.3f)\n",
            median(res["alpha.hat1",], na.rm=TRUE), median(res["alpha.hat2",], na.rm=TRUE),
            median(res["alpha.hat3",], na.rm=TRUE), median(res["alpha.hat4",], na.rm=TRUE)))
cat(sprintf("\nelapsed: %.0fs\nDONE\n", elapsed))
