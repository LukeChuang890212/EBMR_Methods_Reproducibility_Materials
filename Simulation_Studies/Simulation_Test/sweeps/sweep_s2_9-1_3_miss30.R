## Sweep cnr cond for M3 alpha fit, scenario 9-1 setting2 miss30 _3 n=2000.
## M3 = u2_z2 + h_alpha=full. (Same M3 as 8-1 per EQUIVALENT_SCENARIOS_MODEL3.)
...
make_ps_spec_M3 <- function(opt, cond) {
  list(formula.list = list(r ~ y + u2 + z2),
...
run_one <- function(opt, cond) {
  ...
  clusterExport(cl, c("nn","W_sm","h_full","inv_link_fn","all_data",
                      "alpha.true","ps_model.true","make_ps_spec_M3","opt","cond"),
                envir = environment())
...
report <- function(tag, out) {
  ...
  xc <- cl$result
  mu_v <- xc[1,]; se_v <- xc[3,]
  ma_full <- r["max_a", ]; ma_v <- ma_full[!is.na(ma_full)]
  ...
  cat(sprintf("%-26s | Bias=%+.4f ESD=%.4f ESE(m)=%.4f ESE(med)=%.4f ratio=%.3f CP=%.3f | max|a| q99=%.2f max=%.2f | blow=%d | rmvd=%d (%.0fs)\n",
