## Stricter cond sweep for s2.B1 n=500: cond ∈ {200, 300, 500, 700}
## Hope: enough room to take first step but tight enough to block separation.
...
n_reps  <- 500
cfg <- list(label = "s2.B1 n=500", dgp = "setting2.B1", n = 500,
            mu_true = get_mu_true("setting2"))
conds <- c(200, 300, 500, 700)
...
cat("\n========= STRICTER SWEEP: s2.B1 n=500, cond ∈ {200, 300, 500, 700} =========\n")
out_all <- list()
for (cond in conds) {
  ...
  cat(sprintf("  cond=%4d | stuck@0=%4d/%d sep(>50)=%3d | Bias=%+.4f ESD=%.4f ESE=%.4f ESE/ESD=%.3f CP=%.3f | sd(a)=(%.2f,%.2f,%.2f,%.2f) max|a|=%.1f (%.0fs)\n",
  ...
  out_all[[as.character(cond)]] <- list(res = res)
}
saveRDS(out_all, "Simulation_Test/sweep_cnr_strict_n500_results.RDS")
cat("\n========= DONE =========\n")
