...
for (model_idx in 2:3) {
  ...
  cat(sprintf("=== Model %d only, %d reps, se1 vs se2 ===\n", model_idx, n_reps))
  ...
  for (i in 1:n_reps) {
    ...
    tryCatch({
      ...
      # Compute se2-based se_ipw: replace psi1 with psi2 in influence function
      # J=1 so no nu adjustment needed
      r_   <- dat[["r"]]
      y_   <- dat[["y"]]
      w_hat <- res[["w.hat"]]
      ens_ps <- as.vector(res[["ps.matrix"]] %*% w_hat)
      alpha_dim <- length(ebmr[["ps_fit.list"]][[1]][["gmm_fit"]][["estimates"]])
      ...
      psi2 <- ebmr[["ps_fit.list"]][[1]][["gmm_fit"]][["psi2"]]
  ...
  cat(sprintf("\n  Errors: %d / %d, Valid: %d, se2 available: %d\n",
      n_err, n_reps, sum(valid), sum(!is.na(se2_v))))
  cat(sprintf("  Bias:  %.4f\n", mean(mu_v) - mu_true))
  cat(sprintf("  ESD:   %.4f\n", esd))
