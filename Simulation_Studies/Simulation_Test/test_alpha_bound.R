...
# Option 1: Box constraint on alpha_y
run_bounded_gmm <- function(dat, design_mat, h_x, link_type, y_col, alpha_y_lb) {
  r_vec <- dat[["r"]]
  ...
  project <- function(alpha) {
    alpha[y_col] <- max(alpha[y_col], alpha_y_lb)
    alpha
  }
  ...
  nr_inner <- function(start, W_hat, trust_r = 2.0) {
    ...
    for (iter in 1:500) {
      ...
      for (ls in 1:30) {
        cand <- project(alpha + step * direction)
        g_cand <- g_fn(cand)
        G_cand <- matrix(.colMeans(g_cand, n, h_dim), h_dim, 1)
        obj_new <- as.numeric(crossprod(G_cand, W_hat %*% G_cand))
        if (is.finite(obj_new) && obj_new < obj_cur - 1e-4 * step * sum(gv * (cand - alpha))) {
...
# ===== Run for multiple bounds =====
for (lb in c(-3, -5, -7)) {
  cat(sprintf("\n=== Option 1: alpha_y lower bound = %d ===\n", lb))
  ...
  for (rep_i in 1:n_reps) {
    ...
    tryCatch({
      ...
      res <- run_bounded_gmm(dat, design_mat, h_x, link_type, y_col, alpha_y_lb = lb)
      cond_vals[rep_i] <- res$cond
      grad_vals[rep_i] <- res$grad_norm
      alpha_y_vals[rep_i] <- res$estimates[y_col]
      if (abs(res$estimates[y_col] - lb) < 0.01) n_hit_bound <- n_hit_bound + 1
  ...
  cat(sprintf("  Valid: %d/%d, Degenerate: %d, Converged: %d, Hit bound: %d\n",
      sum(valid), n_reps, n_degen, n_conv, n_hit_bound))
