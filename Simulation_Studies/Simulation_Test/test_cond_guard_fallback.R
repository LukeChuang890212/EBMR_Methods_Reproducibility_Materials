...
# with ONE change: cond guard on fallback acceptance
run_cnr_cond_guard <- function(dat, design_mat, h_x, link_type) {
  r_vec <- dat[["r"]]
  ...
  # Inner optimizer: same as package nr_inner
  ...
  if (final_grad_norm >= tol) {
    ...
    # === THE ONE CHANGE: add cond guard to fallback acceptance ===
    if (fb$grad_norm < cnr_grad_norm) {
      ...
      if (fb_cond < cond_limit) {
        ...
        estimates <- best_estimates  # keep cnr's best
      }
    }
  }
  ...
  list(estimates = estimates, grad_norm = final_grad_norm, cond = final_cond,
       ...
       fb_rejected_cond = fb_rejected_cond)
}
...
for (cfg in settings) {
  ...
  for (rep_i in 1:n_reps) {
    ...
    tryCatch({
      res <- run_cnr_cond_guard(dat, di$design_mat, di$h_x, di$link_type)
