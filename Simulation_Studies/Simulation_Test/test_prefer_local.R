...
# Iterative GMM with full inner minimization + prefer-local-solution
# Same as current package constrained_nr, but after convergence:
#   if solution is far from init, prefer the best local solution
run_prefer_local_gmm <- function(dat, design_mat, h_x, link_type,
                                  dist_threshold = 5, local_grad_threshold = 0.1,
                                  trust_r = 2.0) {
  r_vec <- dat[["r"]]
  ...
  # Full inner minimization (same as package)
  ...
  # Track best LOCAL solution (within dist_threshold of init)
  ...
  for (t in 1:100) {
    ...
    # Update best local (within distance threshold of init)
    dist_from_init <- sqrt(sum((est - init)^2))
    if (dist_from_init <= dist_threshold && grad < best_local_grad) {
      best_local_grad <- grad
      best_local_est <- est
    }
  ...
  # Prefer-local-solution logic:
  # If the best overall solution is far from init, prefer local if its grad is acceptable
  final_est <- best_est
  used_local <- FALSE
  ...
  final_grad <- compute_grad(final_est)
...
# ===== Test with different dist/grad thresholds =====
configs <- list(
  list(dist = 5,  grad = 0.1,  label = "dist=5, grad_th=0.1"),
  list(dist = 5,  grad = 0.01, label = "dist=5, grad_th=0.01"),
  list(dist = 3,  grad = 0.1,  label = "dist=3, grad_th=0.1"),
  list(dist = 3,  grad = 0.01, label = "dist=3, grad_th=0.01")
)
...
for (cfg in configs) {
  ...
  for (rep_i in 1:n_reps) {
    ...
    tryCatch({
      ...
      res <- run_prefer_local_gmm(dat, design_mat, h_x, link_type,
                                   dist_threshold = cfg$dist,
                                   local_grad_threshold = cfg$grad)
  ...
  cat(sprintf("  Valid: %d/%d, Degenerate: %d, Converged: %d, Used local: %d\n",
