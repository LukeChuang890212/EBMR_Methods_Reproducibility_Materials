...
# Penalty for E[r/pi - 1] = 0 constraint (enforced via W[1,1] += lambda)
lambda <- 1000

# Exact replica of package constrained_nr + L-BFGS-B fallback,
# with TWO changes:
# 1. Cond guard on fallback acceptance
# 2. Penalty on W[1,1] to enforce E[r/pi-1]=0
run_cnr_constrained <- function(dat, design_mat, h_x, link_type) {
  r_vec <- dat[["r"]]
  n <- nrow(dat); k <- ncol(design_mat); h_dim <- ncol(h_x)
  init <- rep(0, k)

  # Verify first column of h_x is intercept
  stopifnot(all(h_x[, 1] == 1))
    ...
    for (iter in 1:500) {
      ...
      for (ls in 1:30) {
        ...
        if (is.finite(obj_new) && obj_new < obj_cur - 1e-4 * step * sum(gv * direction)) {
          # Cond guard uses ORIGINAL W (not penalized) for degeneracy detection
          W_orig <- W_hat
          W_orig[1, 1] <- W_orig[1, 1] - lambda  # remove penalty for cond check
          if (W_orig[1, 1] < 0) W_orig[1, 1] <- W_hat[1, 1]  # safety
          cc <- check_cond_W(cand, W_orig)
  ...
  # Step 1: W = I with penalty
  W_init <- add_penalty(diag(h_dim))
  estimates <- nr_inner(init, W_init)
  ...
  for (t in 1:max_outer) {
    ...
    W_pen <- add_penalty(W_hat)
    Gamma <- Gamma_fn(estimates)
    cg <- 2 * as.vector(crossprod(Gamma, W_pen %*% G_vec))
      ...
      if (no_improve_count >= stall_limit) {
        if (n_restarts_done < max_restarts) {
          ...
          if (n_restarts_done <= 2L) {
            ...
            W_b <- add_penalty(tryCatch(W_fn(g_b), error = function(e) diag(h_dim)))
          ...
          estimates <- nr_inner(restart_init, W_pen)
          g_rc <- g_fn(estimates)
          G_rc <- matrix(.colMeans(g_rc, n, h_dim), h_dim, 1)
          W_rc <- add_penalty(tryCatch(W_fn(g_rc), error = function(e) diag(h_dim)))
    ...
    estimates <- nr_inner(estimates, W_pen)
  }

  # Final grad norm (using penalized W)
  g_mat_f <- g_fn(estimates)
  G_f <- matrix(.colMeans(g_mat_f, n, h_dim), h_dim, 1)
  W_f <- add_penalty(tryCatch(W_fn(g_mat_f), error = function(e) diag(h_dim)))
  final_grad_norm <- max(abs(2 * as.vector(crossprod(Gamma_fn(estimates), W_f %*% G_f))))

  used_fallback <- FALSE
  fb_rejected_cond <- FALSE

  # L-BFGS-B fallback (if not converged)
  if (final_grad_norm >= tol) {
    ...
    run_lbfgsb_gmm <- function(start_point) {
      ...
      est <- start_point
      # Step 1: W = I + penalty
      st$W.hat <- add_penalty(diag(h_dim))
      opt0 <- optim(est, obj_fn, gr = grad_fn, method = "L-BFGS-B",
                     control = list(maxit = 1000))
      est <- opt0$par
      for (t_fb in 1:max_outer) {
        cache_fn(est)
        st$W.hat <- add_penalty(tryCatch(W_fn(st$cache_g), error = function(e) diag(h_dim)))
      ...
      W_f <- add_penalty(tryCatch(W_fn(g_f), error = function(e) diag(h_dim)))
      list(estimates = est,
  ...
  list(estimates = estimates, grad_norm = final_grad_norm, cond = final_cond,
       ...
       fb_rejected_cond = fb_rejected_cond, constraint_viol = constraint_viol)
}
...
cat(sprintf("=== Constrained GMM: E[r/pi-1]=0 penalty, lambda=%g ===\n\n", lambda))
...
for (cfg in settings) {
  ...
  cat(sprintf("  Fallback used: %d, Fallback rejected (cond): %d\n", n_fb, n_fb_rej))
