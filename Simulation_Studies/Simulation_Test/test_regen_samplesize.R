...
for (n_val in c(500, 1000, 2000, 5000, 10000)) {
  mu_vals <- se2_vals <- numeric(n_target)
  n_tried <- 0; n_done <- 0
  ...
  skip_rate <- 100 * (n_tried - n_target) / n_tried
  cat(sprintf("n=%5d | tried=%d, skipped=%d (%.1f%%) | Bias=%.4f, ESD=%.4f, ESE2=%.4f, ESE2/ESD=%.3f\n",
      n_val, n_tried, n_tried - n_target, skip_rate,
      mean(mu_vals) - mu_true, sd(mu_vals), mean(se2_vals),
