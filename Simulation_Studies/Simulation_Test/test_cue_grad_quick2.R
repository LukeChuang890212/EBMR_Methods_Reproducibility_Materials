...
test_reps <- c(1, 2, 8, 92, 133, 155)

cat("=== Checking inner optim convergence for each PS model ===\n\n")
for (rep_i in test_reps) {
  ...
  tryCatch({
    ...
    cat(sprintf("  Ensemble: nu=(%.4f,%.4f) w=(%.4f,%.4f) conv=%s iter=%d grad=%.2e obj=%.6f last_inner=%s\n",
        res$nu.hat[1], res$nu.hat[2], res$w.hat[1], res$w.hat[2],
        ...
        ens_fit$gmm_fit$opt$last_inner_conv))
