...
# Option 2: ESS constraint — reject steps where y=1 subgroup ESS drops too low
# ESS for y=1 subgroup: measures how many "effective" y=1 respondents remain
# When pi->1 for y=1 obs, weights r/pi -> 1 and moment contrib -> 0
# We use: ESS_y1 = sum(r * (y==1) * (1 - pi)^2 / pi^2) relative to baseline
# Simpler: just check how many y=1 obs have pi > threshold

run_ess_gmm <- function(dat, design_mat, h_x, link_type, y_vec, ess_frac = 0.1) {
  r_vec <- dat[["r"]]
  n <- nrow(dat); k <- ncol(design_mat); h_dim <- ncol(h_x)
  n_y1_resp <- sum(r_vec == 1 & y_vec == 1)  # respondents with y=1
  ...
  # ESS check: effective contribution of y=1 respondents to moment conditions
  # |g_i| for y=1 respondents should not be negligible
  # We measure: mean |r/pi - 1| for y=1 respondents vs y=0 respondents
  # If ratio drops below ess_frac, reject the step
  check_ess <- function(alpha) {
    pi_v <- compute_pi_fn(as.vector(design_mat %*% alpha))
    w <- abs(r_vec / pi_v - 1)
    y1_mask <- (y_vec == 1)
    mean_w_y1 <- mean(w[y1_mask & r_vec == 1])
    mean_w_y0 <- mean(w[!y1_mask & r_vec == 1])
    if (is.na(mean_w_y0) || mean_w_y0 < 1e-10) return(TRUE)
    ratio <- mean_w_y1 / mean_w_y0
    ratio >= ess_frac
  }
  ...
  nr_inner <- function(start, W_hat, trust_r = 2.0) {
    ...
    for (iter in 1:500) {
      ...
      for (ls in 1:30) {
        ...
        # Check ESS constraint BEFORE accepting
        if (!check_ess(cand)) {
          step <- step * 0.5
          next
        }
...
# ===== Test multiple ESS thresholds =====
for (ef in c(0.05, 0.10, 0.20)) {
  cat(sprintf("\n=== Option 2: ESS fraction threshold = %.2f ===\n", ef))
  mu_vals <- se_vals <- cond_vals <- grad_vals <- alpha_y_vals <- rep(NA_real_, n_reps)
  ...
  for (rep_i in 1:n_reps) {
    ...
    tryCatch({
      ...
      y_col <- which(colnames(design_mat) == "y")
      if (length(y_col) == 0) y_col <- 2

      res <- run_ess_gmm(dat, design_mat, h_x, link_type, y_vec, ess_frac = ef)
      cond_vals[rep_i] <- res$cond
      grad_vals[rep_i] <- res$grad_norm
      alpha_y_vals[rep_i] <- res$estimates[y_col]
  ...
  cat(sprintf("  CP       = %.3f\n", cp))
  cat(sprintf("  alpha_y range: [%.3f, %.3f], median=%.3f\n",
      min(alpha_y_vals[valid]), max(alpha_y_vals[valid]), median(alpha_y_vals[valid])))
}
