...
# SGD->NR fallback for degenerate reps only
run_sgd_nr_fallback <- function(dat, design_mat, h_x, link_type) {
  ...
  sgd_inner <- function(start, W_hat, batch_size = 256, max_iter = 1000, lr0 = 0.05) {
  ...
  # Phase 1: SGD to find well-conditioned basin
  ...
  # Phase 2: NR to converge from SGD's basin
  ...
  for (t in 1:100) {
    ...
    if (cc >= cond_limit) {
      retry_ok <- FALSE
      for (tr_scale in c(0.5, 0.25, 0.1)) {
        retry <- nr_inner(prev_est, prev_W, trust_r = 2.0 * tr_scale)
...
# ========== Strategy: NR first, SGD->NR fallback for degenerate ==========
cat("=== Model 3: NR first, SGD->NR fallback for degenerate ===\n")
m_idx <- 3
...
for (rep_i in 1:n_reps) {
  ...
  tryCatch({
    ...
    if (sol_cond < cond_limit) {
      # NR succeeded — use its solution + SE2
      ...
      # NR produced degenerate solution — fallback to SGD->NR
...
cat(sprintf("  Degenerate (after fallback): %d\n", n_degen))
cat(sprintf("  Converged (grad<1e-6): %d\n", n_conv))
...
if (sum(sgd_idx) > 0) {
  cat(sprintf("  [SGD->NR reps] n=%d, mean_mu=%.4f, mean_SE2=%.4f, mean_grad=%.2e, mean_cond=%.2e\n",
      sum(sgd_idx), mean(mu_vals[sgd_idx]), mean(se2_vals[sgd_idx]),
      mean(grad_vals[sgd_idx]), mean(cond_vals[sgd_idx])))
  # Show individual SGD->NR reps
