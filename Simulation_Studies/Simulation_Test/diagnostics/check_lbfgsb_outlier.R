## Test L-BFGS-B method: outlier diagnostics
## Try different bounds to find the sweet spot
...
for (bound in c(20, 10, 5)) {
  cat(sprintf("\n========== L-BFGS-B bounds = [-%d, %d] ==========\n", bound, bound))

  # Temporarily modify the L-BFGS-B bounds by setting method to L-BFGS-B
  # The bounds are hardcoded in GMM_functions.R, so we test with the default [-20,20]
  # For other bounds, we'd need to modify. For now, test with method="L-BFGS-B"
  ...
  for (i in 1:n_reps) {
    ...
    tryCatch({
      ebmr <- EBMRAlgorithmFast4$new("y", ps_spec, dat, W_func, method = "L-BFGS-B")
  ...
  # Only run bound=20 since bounds are hardcoded; break after first iteration
  break
}
