...
run_converged_test <- function(model_idx, optimizer_name) {
  single_ps <- list(
  ...
  if (sum(strict) > 10) {
    cat(sprintf("    FOC<1e-5:      n=%3d, Bias=%.4f, ESD=%.4f, ESE2/ESD=%.3f\n",
        sum(strict), mean(mu_vals[strict]) - mu_true, sd(mu_vals[strict]),
        mean(se2_mu[strict]) / sd(mu_vals[strict])))
  }
}

cat("=== SE2 by convergence status ===\n")
for (m in 1:3) {
  for (opt in c("L-BFGS-B", "constrained_nr")) {
    run_converged_test(m, opt)
  }
}
