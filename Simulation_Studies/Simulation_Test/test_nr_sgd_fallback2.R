...
# Improved SGD->NR fallback: multiple SGD restarts, more iterations
run_sgd_nr_fallback <- function(dat, design_mat, h_x, link_type) {
  r_vec <- dat[["r"]]
  ...
  # SGD inner with more iterations
  sgd_inner <- function(start, W_hat, batch_size = 256, max_iter = 2000, lr0 = 0.05) {
  ...
  # Multiple SGD restarts to find best well-conditioned basin
  best_overall_est <- NULL
  best_overall_grad <- Inf

  for (restart in 1:5) {
    # Random initial point for diversity
    if (restart == 1) {
      init <- rep(0, k)
    } else {
      init <- rnorm(k) * 0.5
    ...
    # SGD phase: W=I then a few W updates
    est <- sgd_inner(init, diag(h_dim), max_iter = 2000, lr0 = 0.05)
    for (t in 1:5) {
      g_mat <- g_fn(est)
      W_hat <- tryCatch(W_fn(g_mat), error = function(e) diag(h_dim))
      est <- sgd_inner(est, W_hat, max_iter = 1000, lr0 = 0.03)
    }

    # NR phase to converge
    ...
    for (t in 1:50) {
      ...
      if (cc >= cond_limit) {
        # Try NR with smaller trust radius
        retry_ok <- FALSE
        for (tr in c(1.0, 0.5, 0.2)) {
    ...
    # Keep if well-conditioned and better grad
  ...
  # If no well-conditioned solution found, use best from any restart
  if (is.null(best_overall_est)) {
    best_overall_est <- rep(0, k)  # fallback to init
    best_overall_grad <- Inf
  }
...
# ========== NR first, SGD->NR fallback with multiple restarts ==========
cat("=== Model 3: NR + SGD->NR fallback (5 restarts) ===\n")
m_idx <- 3
...
for (rep_i in 1:n_reps) {
  ...
  tryCatch({
    ...
    if (sol_cond < cond_limit) {
      ...
      method_used[rep_i] <- "SGD->NR"
      design_mat <- ebmr$ps_fit.list[[1]]$design_matrix
      h_x <- ebmr$ps_fit.list[[1]]$h_x
      link_type <- ebmr$ps_fit.list[[1]]$link_type

      set.seed(rep_i * 100)
      res <- run_sgd_nr_fallback(dat, design_mat, h_x, link_type)
...
n_nr <- sum(method_used == "NR", na.rm = TRUE)
n_sgd <- sum(method_used == "SGD->NR", na.rm = TRUE)
n_degen <- sum(cond_vals[valid] >= cond_limit, na.rm = TRUE)

cat(sprintf("\n  Valid: %d/%d\n", sum(valid), n_reps))
cat(sprintf("  Method: NR=%d, SGD->NR=%d\n", n_nr, n_sgd))
cat(sprintf("  Still degenerate after fallback: %d\n", n_degen))
cat(sprintf("  Bias  = %.4f\n", mean(mu_vals[valid]) - mu_true))
cat(sprintf("  ESD   = %.4f\n", sd(mu_vals[valid])))
cat(sprintf("  ESE2  = %.4f\n", mean(se2_vals[valid])))
cat(sprintf("  ESE2/ESD = %.3f\n", mean(se2_vals[valid]) / sd(mu_vals[valid])))

nr_idx <- valid & method_used == "NR"
sgd_idx <- valid & method_used == "SGD->NR"
if (sum(nr_idx) > 1) {
  cat(sprintf("\n  [NR reps] n=%d, Bias=%.4f, ESD=%.4f, ESE2=%.4f, ESE2/ESD=%.3f\n",
      sum(nr_idx), mean(mu_vals[nr_idx]) - mu_true, sd(mu_vals[nr_idx]),
      mean(se2_vals[nr_idx]), mean(se2_vals[nr_idx]) / sd(mu_vals[nr_idx])))
}
if (sum(sgd_idx) > 0) {
  cat(sprintf("\n  [SGD->NR reps] n=%d\n", sum(sgd_idx)))
  for (i in which(sgd_idx)) {
    cat(sprintf("    Rep %3d: mu=%.4f, se2=%.4f, grad=%.2e, cond=%.2e\n",
        i, mu_vals[i], se2_vals[i], grad_vals[i], cond_vals[i]))
  }
  sgd_wc <- sgd_idx & cond_vals < cond_limit
  sgd_dg <- sgd_idx & cond_vals >= cond_limit
  cat(sprintf("    Well-conditioned: %d, Still degenerate: %d\n",
      sum(sgd_wc), sum(sgd_dg)))
}
