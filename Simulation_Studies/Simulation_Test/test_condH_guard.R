...
run_condH_guard <- function(setting, miss, ps_spec_id, model_idx, cond_threshold = 1e8) {
  ...
  p_dim <- length(all.vars(formula_j))  # approximate
  ...
  for (rep_i in 1:n_reps) {
    ...
    tryCatch({
      ...
      # Check if alpha is in degenerate region
      is_degenerate <- function(alpha) {
      ...
      # Guarded inner optimizer with callback-style step rejection
      inner_opt_guarded <- function(start, W_hat) {
        ...
        # Check degeneracy at candidate
        Gamma_hat <- Gamma_fn(candidate)
        H <- crossprod(Gamma_hat, W_hat %*% Gamma_hat)
        eig <- eigen(H, symmetric = TRUE, only.values = TRUE)$values
        cond_num <- max(eig) / max(min(eig), 1e-15)

        if (cond_num > cond_threshold) {
          # Reject: candidate is in degenerate flat region
          # Try bisection: find the farthest point along start->candidate that is non-degenerate
          for (frac in c(0.5, 0.25, 0.1, 0.05)) {
            mid <- start + frac * (candidate - start)
            Gamma_mid <- Gamma_fn(mid)
            H_mid <- crossprod(Gamma_mid, W_hat %*% Gamma_mid)
            eig_mid <- eigen(H_mid, symmetric = TRUE, only.values = TRUE)$values
            cond_mid <- max(eig_mid) / max(min(eig_mid), 1e-15)
            if (cond_mid < cond_threshold) {
              return(list(par = mid, guarded = TRUE))
            }
          }
          # All fractions degenerate — stay at start
      ...
      # Check step 1
      if (is_degenerate(estimates)) {
        guard_count <- guard_count + 1L
        # Bisect from zero to estimates
        for (frac in c(0.5, 0.25, 0.1, 0.05)) {
          mid <- frac * estimates
          if (!is_degenerate(mid)) { estimates <- mid; break }
        }
        if (is_degenerate(estimates)) estimates <- rep(0, p)
      }

      # === Step 2+: iterative GMM with guard ===
      best_grad <- Inf
      best_est <- estimates
      no_improve <- 0L
      converged <- FALSE
      ...
      for (t in 1:200) {
        ...
        result <- inner_opt_guarded(estimates, W_hat)
        if (result$guarded) guard_count <- guard_count + 1L
  ...
  cat(sprintf("Valid: %d, Converged: %d, Guard triggered (total): %d\n", sum(valid), sum(conv), guard_count))
  ...
  formula_j_local <- ps_spec[["formula.list"]][[model_idx]]
  for (i in which(valid)) {
    dat <- all_data[((i-1)*n_val + 1):(i*n_val), ]
    dm <- model.matrix(formula_j_local, data=dat)
    alpha_i <- c(mu_vals[i], alpha2_vals[i])  # incomplete, skip pi check
  }
}

# Setting4, Model 3
run_condH_guard("setting4", "miss50", "9-alt1", 3, cond_threshold = 1e8)

# Setting3, Model 2
run_condH_guard("setting3", "miss50", "9-alt1", 2, cond_threshold = 1e8)
