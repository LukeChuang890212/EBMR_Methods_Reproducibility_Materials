...
# Hybrid SGD → NR iterative GMM
run_hybrid_gmm <- function(dat, design_mat, h_x, link_type) {
  r_vec <- dat[["r"]]
  ...
  # --- Phase 1: SGD to find basin (few outer iters, many inner iters) ---
  ...
  # --- Phase 2: NR to converge precisely from SGD's basin ---
  ...
  # NR phase: iterative W updates from SGD solution
  ...
  for (t in 1:100) {
    ...
    if (cc >= cond_limit) {
      # Revert and retry with smaller trust radius
      retry_ok <- FALSE
...
# ========== Run M3 on all 200 reps ==========
cat("=== Model 3: Hybrid SGD->NR ===\n")
m_idx <- 3
...
for (rep_i in 1:n_reps) {
  ...
  tryCatch({
    ...
    set.seed(rep_i)
    res <- run_hybrid_gmm(dat, design_mat, h_x, link_type)
...
cat(sprintf("\n  Valid: %d/%d, Degenerate: %d, Converged (grad<1e-6): %d\n",
...
conv <- valid & grad_vals < 1e-4
if (sum(conv) > 1) {
  cat(sprintf("  [Converged grad<1e-4] n=%d, Bias=%.4f, ESD=%.4f, ESE2=%.4f, ESE2/ESD=%.3f\n",
      sum(conv), mean(mu_vals[conv]) - mu_true, sd(mu_vals[conv]),
      mean(se2_vals[conv]), mean(se2_vals[conv]) / sd(mu_vals[conv])))
}
