...
simulate = function(all_data, ps_model.true, alpha.true, ps_specifications, n, replicate_num, save.file = NULL){
  ...
  sim_result = foreach(i = 1:replicate_num, .combine = 'cbind', .options.snow = opts, .packages = parallel_packages) %dopar% {
      ...
      result = ebmr$EBMR_IPW(h_x_names = c("u1", "u2"),
                             true_ps = ps_model.true(y, u1, u2, r, alpha.true))
  ...
  for(n in n.vector){
    for(model_num in 1:J){
      ...
      for(i in 1:ncol(model_combinations)){
        ...
        save.file = paste0(c("Simulation_Results/EBMR_IPW_",scenario, "_", model_set, "_", n, "_", replicate_num, "_", version, ".RData"), collapse = "")
        ...
        simulate(all_data, ps_model.true, alpha.true, ps_specifications[model_set], n, replicate_num, save.file)
      }
    }
  }
}
...
simulate_all_model_combinations_and_sample_sizes(
  ...
  n.vector = n.vector,
  all_data_file = "Simulation_Data/ChuangData_SM1.1_2000.RDS",
  scenario = 1,
  version = "43",
