...
dat$y = dat$father-dat$parent_report
...
mu_cc = mean(dat$y[dat$r == 1])
se_cc = sd(dat$y[dat$r == 1])/sqrt(sum(r))
```
...
full_ps_specifications = list(
  ...
  h_x_names.list = c("father", "parent_report", "health"),
...
for(model_num in 1:4){
  model_combinations = combn(4, model_num)
  for(i in 1:ncol(model_combinations)){
    ...
    ps_specifications <- list(
        ...
        h_x_names.list = full_ps_specifications$h_x_names,
    ...
    for(j in 1:J){
     formula = ps_specifications$formula.list[[j]]
     h_x_names = ps_specifications$h_x_names
     inv_link = ps_specifications$inv_link
     ps_fit.list[[j]] = WangShaoKim2014(formula, h_x_names, inv_link, W = W, data = dat)
    }

    result = EBMR_IPW(h_x_names = c("father", "parent_report", "health"), W = W, data = dat)
    data = subdat
    h_x = data[h_x_names]
    ...
    save_file = paste0(c("MHD_results/EBMR_IPW_diff_", model_set, "_OR4.RDS"), collapse = "")
