...
estimate_nu = function(alpha, alpha_dim, ps_model.list, h_x, data, init = NULL){
  r = as.matrix(data$r)
  y = as.matrix(data$y)
  n = nrow(data)
...
ensemble = function(ps_fit.list, h_x, data, init = NULL, se.fit) {
  ...
  nu.hat = estimate_nu(alpha.hat, alpha_dim, ps_model.list, h_x, data, init)
...
EBMR_IPW = function(h_x_names, data, se.fit = TRUE, true_ps = NULL) {
  # Basic setup
  r = as.matrix(data$r)
  y = as.matrix(data$y)
  n = nrow(data)
  ...
  # Ensemble step
  ################################################################################
  ensemble_fit = ensemble(ps_fit.list, data[h_x_names], data, init = rep(1/J, J), se.fit)
...
WangShaoKim2014_perturb = function(formula, h_x_names, inv_link, data, wt, init = NULL, se.fit = TRUE) {
  # Basic setup
  result = parse_formula(formula)
  r = as.matrix(data[result$r_names])
  y = as.matrix(data[result$y_names])
...
estimate_nu_perturb = function(alpha, alpha_dim, ps_model.list, h_x, data, wt, init = NULL){
  r = as.matrix(data$r)
  y = as.matrix(data$y)
  n = nrow(data)
  J = length(ps_model.list )
...
ensemble_perturb = function(ps_fit.list, h_x, data, wt, init = NULL, se.fit) {
  alpha.list = lapply(ps_fit.list, function(ps_fit) ps_fit$coefficients)
  alpha.hat = unlist(alpha.list)
  alpha_dim = unlist(lapply(alpha.list, length))
  ps_model.list = lapply(ps_fit.list, function(ps_fit) ps_fit$model)
  ...
  nu.hat = estimate_nu_perturb(alpha.hat, alpha_dim, ps_model.list, h_x, data, wt, init)
...
EBMR_IPW_perturb = function(h_x_names, data, wt, se.fit = TRUE, true_ps = NULL) {
  # Basic setup
  r = as.matrix(data$r)
  y = as.matrix(data$y)
  n = nrow(data)
  ...
  # Ensemble step
  ################################################################################
  ensemble_fit = ensemble_perturb(ps_fit.list, data[h_x_names], data, wt, init = rep(1/J, J), se.fit)
  # ensemble_fit = estimate_nu(ps.matrix, data[h_x_names], data, init = rep(1/J, J))
  nu.hat = ensemble_fit$coefficients
  # W = function(v) v^2/sum(v^2)
  # V_inv = if (J == 1) {
