...
#' the response indicator variable (`r`), the variable(s) inside `o()`,
...
#' Multiple `o()` terms are supported (e.g., `r ~ o(y) + o(y*x) + x`).
#'
...
#'   \item The missing outcome variable(s) inside `o()`, if any. Multiple `o()` terms are supported.
...
#'
#' # Multiple o() terms
#' parsed_formula <- ebmr$parse_formula(r ~ o(y) + o(y*health) + health + father)
#' print(parsed_formula)
#' }
#'
#' @keywords internal
...
parse_formula = function(formula) {
  ...
  # Extract ALL variables inside o() using regular expressions (support multiple o() terms)
  y_matches <- str_extract_all(rhs, "o\\(([^)]+)\\)")[[1]]

  if(length(y_matches) == 0){
    y_names = character(0)
  }else{
    # Clean up: Remove 'o()' wrapper from each match
    y_names <- gsub("o\\(|\\)", "", y_matches)
    y_names <- trimws(y_names)
  }


  # Extract the rest of the variables (excluding all o() terms)
  ...
  # Remove empty strings that may result from consecutive + signs
  x_names <- x_names[x_names != ""]
